...
class MixOfIndependentSpaces(ObjectiveFunction):
    ...
    def defineDomains(self, continuosDimension: int = 2, numberOfContinuosMinima: int = 2, discretDimension: int = 5, numberOfDiscretMinima: int = 2, distance = "K", difficult="E") -> None:
        ...
        self._continuos = QuadraticFunction(dimension=continuosDimension, numberOfLocalMinima= numberOfContinuosMinima, minima_proximity=10)

        self.optima = []

        for minimum_i in self._discret.optima:
            for minimum_j in self._continuos.minimas:
    ...
    def log(self):
        ...
        for p, p_optimum in enumerate(self._discret.optima):
            for c, c_optimum in enumerate(self._continuos.minimas):
                print(f"{self._continuos.p_list[c]}&{self._discret.permutation.consensus[p]}&{c_optimum:.6}&{p_optimum:.6}&{self.transform(p_optimum,c_optimum):.6}+")
